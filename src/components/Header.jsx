...
export default function Header() {
  ...
  return <>
    ...
    </div><nav ref={nav} id="mobile-navigation" className="renew-mobile-nav" hidden={!open} aria-label={en ? 'Mobile navigation' : '모바일 메뉴'}>{[...links, ['/contact', '비즈니스 문의', 'Contact']].map(([url, ko, eng]) => <NavLink key={url} to={url} onClick={() => {setOpen(false);if(url==="/")window.scrollTo({top:0,behavior:"instant"});}}>{en ? eng : ko}<span aria-hidden="true">↗</span></NavLink>)}</nav></header>
